var express = require('express'), app = express(), server = require('http').createServer(app), io = require('socket.io').listen(server), conf = require('../config.json');

// Imports
var Commands = require('./modules/CommandList');
var GameServer = require('./GameServer');

// Init variables
var showConsole = true;

// Start msg
console.log("[Game] Ogar - An open source Agar.io server implementation");

// Handle arguments
process.argv.forEach(function(val) {
	if (val == "--noconsole") {
		showConsole = false;
	} else if (val == "--help") {
		console.log("Proper Usage: node index.js");
		console.log("    --noconsole         Disables the console");
		console.log("    --help              Help menu.");
		console.log("");
	}
});

// Run Ogar
var gameServer = new GameServer();
gameServer.start();
// Add command handler
gameServer.commands = Commands.list;
// Initialize the server console
if (showConsole) {
	var readline = require('readline');
	var in_ = readline.createInterface({
		input : process.stdin,
		output : process.stdout
	});
	setTimeout(prompt, 100);
}


// Console functions
function prompt() {
	in_.question("", function(str) {
		parseCommands(str);
		return prompt();
		// Too lazy to learn async
	});
};

function parseCommands(str) {
	// Splits the string
	var split = str.split(" ");

	// Process the first string value
	var first = split[0].toLowerCase();

	// Get command function
	var execute = gameServer.commands[first];
	if ( typeof execute != 'undefined') {
		execute(gameServer, split);
	} else {
		console.log("[Console] Invalid Command!");
	}
};


// --------------- MASTERSERVER STUFF --------------------
// --------------- MASTERSERVER STUFF --------------------
// --------------- MASTERSERVER STUFF --------------------
// --------------- MASTERSERVER STUFF --------------------
server.listen(conf.infoPort);
var Connected = false;
var socket = require('socket.io-client')('http://localhost:4443');
socket.on('connect', function() {
	console.log("[INFO] Registerd to masterserver.");
	Connected = true;
});
socket.on('request', function() {
	socket.emit('server', {
		infoPort : conf.infoPort
	});
});
socket.on('disconnect', function() {
	Connected = false
});
setInterval(checkConnection, 5000);
function checkConnection() {
	if (!Connected)
		console.log("[WARNING] Failed to connect to masterserver");
}

io.sockets.on('connection', function(socket) {
	 humans = 0, bots = 0;
        for (var i = 0; i < gameServer.clients.length; i++) {
            if ('_socket' in gameServer.clients[i]) {
                humans++;
            } else {
                bots++;
            }
        }
	socket.emit('info', {
		serverName: conf.serverName,
		serverPort: gameServer.config.serverPort,
		serverGamemode: gameServer.config.serverGamemode,
		serverMaxConnections: gameServer.config.serverMaxConnections,
		serverPort: gameServer.config.serverPort,
		player: humans,
		bot: bots
	});
});

// --------------- MASTERSERVER STUFF --------------------
// --------------- MASTERSERVER STUFF --------------------
// --------------- MASTERSERVER STUFF --------------------
// --------------- MASTERSERVER STUFF --------------------